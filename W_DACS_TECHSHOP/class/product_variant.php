...
class ProductVariant extends DB {
  ...
  /**
   * Lấy variant theo attributes combination
   */
  public function getVariantByAttributes($productId, $attributes) {
    $productId = $this->db_escape($productId);
    ...
    foreach ($attributes as $attributeId => $valueId) {
      ...
      $attributeConditions[] = "(va.attribute_id = '$attrId' AND va.value_id = '$valId')";
    }

    $conditionsStr = implode(' AND ', $attributeConditions);
    $attributeCount = count($attributes);

    $query = "SELECT v.*, COUNT(va.id) as match_count
              FROM {$this->table} v
              LEFT JOIN variant_attributes va ON v.id = va.variant_id
              WHERE v.product_id = '$productId' AND ($conditionsStr)
              GROUP BY v.id
              HAVING match_count = $attributeCount
              LIMIT 1";
